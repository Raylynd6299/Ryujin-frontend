...
import type { IncomeFormValues } from '../components/incomes/IncomeForm';
...
export const IncomesPage = () => {
    ...
    const { data, isLoading } = useIncomes({ page, limit: 20 });
        ...
        <div className="space-y-6">
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                    <p className="text-muted-foreground">{t('finance.noIncomes')}</p>
                    <Button className="mt-4" onClick={() => setOpen(true)}>
                        <Plus className="mr-2 h-4 w-4" />
                        {t('finance.addFirstIncome')}
                    </Button>
                </div>
            )}
        </div>
